 ...
 * monolitna verzija je citala likvidni cash fonda direktno preko
 * {@code accountRepository.findById(fund.accountId).map(Account::getBalance)}.
 * U trading-service-u racuni zive u banka-core domenu, pa se balans fond
 * racuna cita preko banka-core internog API-ja
 * ({@link BankaCoreClient#getAccount}). Ostatak racuna (hartije fonda) je
 * lokalni {@link Portfolio} — kopiran verbatim.
 */
@Service
