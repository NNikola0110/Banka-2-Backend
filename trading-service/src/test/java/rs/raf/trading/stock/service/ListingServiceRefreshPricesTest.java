 ...
 * Covers: AlphaVantage mock, BankaCoreClient (FX) mock, fallback simulation, FUTURES random, price history snapshots.
 */
...
class ListingServiceRefreshPricesTest {
    ...
    // ─── refreshPrices: FOREX via BankaCoreClient (fixer.io seam) ───────────────

    @Nested
    @DisplayName("refreshPrices - FOREX with BankaCoreClient")
    class ForexRefresh {
        ...
        void forexCrossRateCalculation() {
            ...
            List<FxRateDto> rates = List.of(
                    new FxRateDto("EUR", 0.008547),  // 1 RSD = 0.008547 EUR
                    new FxRateDto("USD", 0.009090)   // 1 RSD = 0.009090 USD
            );
            when(bankaCoreClient.getFxRates()).thenReturn(rates);
        ...
        @Test
        @DisplayName("forex fallback when BankaCoreClient throws exception")
        void forexFallbackOnException() {
            Listing forex = forexListing("GBP", "JPY", BigDecimal.valueOf(180));
            when(listingRepository.findAll()).thenReturn(List.of(forex));
            when(dailyPriceRepository.findByListingIdAndDate(any(), any())).thenReturn(Collections.emptyList());
            when(bankaCoreClient.getFxRates()).thenThrow(new RuntimeException("Fixer API unavailable"));
        ...
        void forexFallbackOnMissingCurrency() {
            ...
            List<FxRateDto> rates = List.of(new FxRateDto("USD", 0.009090));
            when(bankaCoreClient.getFxRates()).thenReturn(rates);
    ...
    class EdgeCases {
        ...
        void mixedListings() {
            ...
            when(restTemplate.getForObject(any(String.class), eq(Map.class))).thenReturn(null);
            // Forex: BankaCoreClient throws -> fallback
            when(bankaCoreClient.getFxRates()).thenThrow(new RuntimeException("API error"));
